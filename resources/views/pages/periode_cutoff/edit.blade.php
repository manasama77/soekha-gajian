                ...
                <form action="{{ route('setup.periode-cutoff.update', $periode_cutoff) }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label for="start_date"
                            class="dark:text-white block mb-2 text-sm font-medium text-gray-900">Start Date</label>
                        <input type="date" name="start_date" id="start_date"
                            ...
                            value="{{ $periode_cutoff->start_date->format('Y-m-d') }}" required />
                        @error('start_date')
                            <p class="text-xs italic text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-4">
                        <label for="end_date" class="dark:text-white block mb-2 text-sm font-medium text-gray-900">End
                            Date</label>
                        <input type="date" name="end_date" id="end_date"
                            class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('name') border-red-500 @enderror"
                            value="{{ $periode_cutoff->end_date->format('Y-m-d') }}" required />
                        @error('end_date')
                            <p class="text-xs italic text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
